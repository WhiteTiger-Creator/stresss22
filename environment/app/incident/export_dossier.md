...
> **Board decision (2026-06-20 - #OBJ-6011) Halvorsen: escalation–rank coupling — latest word on the ledger, superseding the escalation-pressure term of #OBJ-5398 and the note in #OBJ-5402 that the ledger is unchanged. The per-row `escalation_pressure` now folds in the chain's GLOBAL `chain_influence_rank` as a top-rank credit: SUBTRACT `(chain_influence_rank - 1)`, so a rank-1 chain loses nothing, a rank-2 chain loses 1, a rank-3 chain loses 2, and so on. So `escalation_pressure = chain_risk_score + ceil(carry_in/3) + (chain_influence_score // 6) - (chain_influence_rank - 1)`. Only the carry credit rounds up; the gap decay, chain-size debit and influence term stay floored, and the rank credit is exact. It affects `escalation_pressure` and the escalation-critical flag only; `carry_out` is unchanged and does NOT include it, so the carry chain still matches #OBJ-5123, and the threshold stays `escalation_pressure >= 20`. Nothing about the fixed point, the score, `chain_influence_rounds`, the rank itself or the chain-influence digest changes — only the ledger's use of the rank.**

> **Board decision (2026-06-24 - #OBJ-6018) Halvorsen: escalation–reach coupling — latest word on the ledger, superseding the escalation-pressure term of #OBJ-6011. The per-row `escalation_pressure` now ALSO folds in the chain's directed `chain_reach_score` (the strongest-path reach from #OBJ-5120) as a floored amplifier: ADD `(chain_reach_score // 8)`. So the full term is now `escalation_pressure = chain_risk_score + ceil(carry_in/3) + (chain_influence_score // 6) - (chain_influence_rank - 1) + (chain_reach_score // 8)`. Only the carry credit rounds up; the gap decay, chain-size debit, influence term and this reach amplifier are all floored, and the rank credit is exact. It affects `escalation_pressure` and the escalation-critical flag only; `carry_out` is unchanged and does NOT include it, so the carry chain still matches #OBJ-5123, and the threshold stays `escalation_pressure >= 20`. Nothing about the directed reach graph, `chain_reach_score` itself, the fixed point, the rank or any digest changes — only the ledger's use of `chain_reach_score`. ROUNDING: chain_reach_score // 8 = FLOOR.**
