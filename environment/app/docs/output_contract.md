# Bucketwatch signal recovery contract
...
Create `/app/object_audit.py` with these commands:
...
A diagnosed report contains only `pipeline_status`, `issues_found`, and `input_stats`, with status `diagnosed`. `input_stats` carries exactly the keys `object_count`, `unique_object_ids`, and `buckets` — no aliases. It does not contain `verified_summary` or `output_paths`. A repaired report has status `repaired`, embeds the generated summary, and uses the semantic path keys `summary_json`, `escalated_jsonl`, and `bucket_matrix_json`.

## Signal processing

The workflow canonicalizes objects and deterministically deduplicates them by `object_id`. Muted or actively overridden candidates do not enter the triage queue. Override windows are normalized and compacted before overlap and pressure calculations.

Related objects form transitive chains. Chains then participate in a directed reach graph whose strongest path is propagated in deterministic order. Chain reach affects final row ordering, signal digests, and summary checksums. `report_spec.json` defines the schemas, checksum serialization, and digest payloads; the normalization rules, tie-break cascade, interval boundaries, edge rules, and equations themselves were settled during the Bucketwatch review and are recorded as #OBJ-ticketed decision notes in the incident dossier — reconcile them there, and note the February triage proposals were partly reversed later.
...
`removed_tokens` maps each forbidden token string to a boolean. The removed-token map uses the exact source literals `event["accessed_at"]` and `severity == "critical"`. The post-repair section objects integer `escalated_count` and `rerun_escalated_count`.
