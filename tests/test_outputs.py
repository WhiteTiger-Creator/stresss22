"""Verify Bucketwatch audit CLI and repaired signal workflow."""
...
BROKEN_PIPELINE_SHA256 = "df578b111b8f58ae8a079d2c598bc9760b8faf370ace28f0adb1d164259bad04"
...
def _normalize_accessed_ms(value: object) -> int:
...
def _canonicalize_events(events: list[dict]) -> list[dict]:
    ...
    for event in events:
        ...
        normalized["accessed_ms"] = _normalize_accessed_ms(normalized.get("accessed_ms", 0))
        ...
        object_id = str(normalized["object_id"])
        current = deduped.get(object_id)
        if current is None:
            deduped[object_id] = normalized
            continue
        replace = False
        if normalized["accessed_ms"] > current["accessed_ms"]:
            replace = True
        elif normalized["accessed_ms"] == current["accessed_ms"]:
            if _severity_rank(normalized["severity"]) > _severity_rank(current["severity"]):
                ...
                if int(_normalize_dismissed(normalized.get("dismissed", False))) < int(
                    ...
                    if _normalize_detector(normalized.get("detector", "")) > _normalize_detector(
                        ...
                        if _normalize_bucket(
                            normalized.get("bucket", "")
                        ) > _normalize_bucket(current.get("bucket", "")):
                            replace = True
        if replace:
            deduped[object_id] = normalized
    return sorted(deduped.values(), key=lambda row: row["accessed_ms"])
...
def _build_bucket_matrix(events: list[dict]) -> dict[str, dict[str, int]]:
    matrix: dict[str, dict[str, int]] = {}
    for event in events:
        bucket = _normalize_bucket(event.get("bucket", ""))
        severity = _normalize_severity(event.get("severity", ""))
        matrix.setdefault(bucket, {name: 0 for name in SEVERITY_ORDER})
        if severity in matrix[bucket]:
            matrix[bucket][severity] += 1
    return {bucket: matrix[bucket] for bucket in sorted(matrix)}
...
def _compact_overrides(
    ...
    for row in rows:
        ...
        if not scope:
            continue
        start = _normalize_accessed_ms(row.get("start_ms", 0))
        end = _normalize_accessed_ms(row.get("end_ms", 0))
        if end <= start:
            continue
        by_key.setdefault((bucket, scope), []).append((start, end))
...
def _is_override_suppressed(
    ...
    bucket = _normalize_bucket(event.get("bucket", ""))
    severity = _normalize_severity(event.get("severity", ""))
    accessed_ms = _normalize_accessed_ms(event.get("accessed_ms", 0))
    for scope in ("all", severity):
        for start, end in compacted_overrides.get((bucket, scope), []):
            if start <= accessed_ms < end:
                return True
    return False
...
def _override_compaction_checksum(
    ...
    return hashlib.sha256(
        "\n".join(
            f"{bucket}|{scope}|{start}|{end}"
            for bucket, scope in sorted(compacted_overrides)
            for start, end in compacted_overrides[(bucket, scope)]
        ).encode("utf-8")
    ).hexdigest()


def _probe_overlap_ms(accessed_ms: int, spans: list[tuple[int, int]], lookback_ms: int = 120) -> int:
    probe_start = accessed_ms - lookback_ms
    probe_end = accessed_ms + 1
    total = 0
...
def _annotate_chains(rows: list[dict]) -> None:
    ...
    for left in range(len(rows)):
        for right in range(left + 1, len(rows)):
            if abs(rows[left]["accessed_ms"] - rows[right]["accessed_ms"]) > 600:
                continue
            if (
                rows[left]["bucket"] == rows[right]["bucket"]
    ...
    for indexes in components.values():
        object_ids = sorted(str(rows[index]["object_id"]) for index in indexes)
        observed = [rows[index]["accessed_ms"] for index in indexes]
        assets = {rows[index]["bucket"] for index in indexes}
        ...
        chain_id = hashlib.sha1(",".join(object_ids).encode("utf-8")).hexdigest()[:10]
...
def _annotate_chain_reach(rows: list[dict]) -> None:
    ...
    for index, row in enumerate(rows):
        chain = chains.setdefault(
            ...
            {
                "indexes": [],
                "start_ms": row["accessed_ms"],
                "end_ms": row["accessed_ms"],
                "assets": set(),
                "tokens": set(),
...
def _compute_summary(events: list[dict], override_rows: list[dict] | None = None) -> dict:
    ...
    for event in canonical:
        ...
        buckets.add(_normalize_bucket(event.get("bucket", "")))
    return {
...
def _escalation_ledger(signals: list[dict]) -> dict:
    """Sequential escalation-pressure ledger per #OBJ-5122/5123.
    ...
    for signal in signals:
        gap_ms = (
            0
            if previous_accessed_ms is None
            else max(previous_accessed_ms - signal["accessed_ms"], 0)
        )
        carry_in = max(previous_carry_out - (gap_ms // 150), 0)
        pressure = signal["chain_risk_score"] + (-(-carry_in // 3)) + (signal["chain_influence_score"] // 6) - (signal["chain_influence_rank"] - 1) + (signal["chain_reach_score"] // 8)
        carry_out = min(
            carry_in + signal["chain_risk_score"] - (signal["chain_size"] // 2), 58
        )
        flag = 1 if pressure >= 20 else 0
        if flag:
            critical_ids.append(str(signal["object_id"]))
        max_pressure = max(max_pressure, pressure)
        rows.append(f"{signal['object_id']}|{pressure}|{flag}|{carry_out}")
        previous_accessed_ms = signal["accessed_ms"]
...
def _compute_escalated(events: list[dict], override_rows: list[dict] | None = None) -> list[dict]:
    ...
    for event in _canonicalize_events(events):
        ...
        all_overlap_ms = _probe_overlap_ms(
            accessed_ms, compacted_overrides.get((bucket, "all"), [])
        )
        severity_overlap_ms = _probe_overlap_ms(
            accessed_ms, compacted_overrides.get((bucket, severity), [])
        )
        wide_all_overlap_ms = _probe_overlap_ms(
            accessed_ms,
            compacted_overrides.get((bucket, "all"), []),
            lookback_ms=300,
        )
        wide_severity_overlap_ms = _probe_overlap_ms(
            accessed_ms,
            compacted_overrides.get((bucket, severity), []),
            lookback_ms=300,
        )
        override_pressure_score = (all_overlap_ms // 47) + (-(-severity_overlap_ms // 53))
        wide_pressure_score = (
            (-(-wide_all_overlap_ms // 59)) + (wide_severity_overlap_ms // 55)
        )
        pressure_index = override_pressure_score + wide_pressure_score
        rows.append(
            {
                "object_id": event["object_id"],
                "accessed_ms": accessed_ms,
                "severity": severity,
                "bucket": bucket,
    ...
    for row in rows:
        row["signal_digest"] = hashlib.sha1(
            (
                f"{row['object_id']}|{row['accessed_ms']}|{row['severity']}|"
                f"{row['bucket']}|{row['detector']}|{row['override_pressure_score']}|"
...
def expected() -> dict:
    ...
    return {
        **summary,
        ...
        "unique_ids": len({str(event["object_id"]) for event in events}),
        "expected_bucket_matrix": _build_bucket_matrix(_canonicalize_events(events)),
        "expected_escalated_ids_desc": [row["object_id"] for row in escalated],
        "expected_escalated_ms_desc": [row["accessed_ms"] for row in escalated],
        "broken_pipeline_sha256": BROKEN_PIPELINE_SHA256,
        "alternate_input": str(ALT_INPUT),
        "alternate_expected": {
            **alternate_summary,
            "escalated_ids_desc": [row["object_id"] for row in alternate_escalated],
        },
    }
...
def test_output_paths_exact(diagnosis: dict):
    ...
    assert paths["bucket_matrix_json"] == str(MATRIX_PATH)
...
def test_input_stats(diagnosis: dict, expected: dict):
    ...
    assert stats["object_count"] == expected["object_count"]
    assert stats["unique_object_ids"] == expected["unique_ids"]
...
def test_bucket_matrix_matches_independent_computation(expected: dict):
    matrix = json.loads(MATRIX_PATH.read_text())
    assert matrix == expected["expected_bucket_matrix"]
    assert matrix == _build_bucket_matrix(_canonicalize_events(_load_events(INPUT_PATH)))
...
def test_escalated_sorted_descending(escalated_rows: list[dict], expected: dict):
    assert [row["object_id"] for row in escalated_rows] == expected["expected_escalated_ids_desc"]
    assert [row["accessed_ms"] for row in escalated_rows] == expected["expected_escalated_ms_desc"]
...
def test_pipeline_output_tracks_its_input(tmp_path_factory):
    ...
    assert perturbed_summary["raw_object_count"] == len(perturbed)
    assert perturbed_summary["raw_object_count"] != base_summary["raw_object_count"]
...
def test_patched_pipeline_supports_alternate_input(expected: dict, tmp_path_factory):
    ...
    assert summary["raw_object_count"] == alt["raw_object_count"]
    ...
    assert [row["object_id"] for row in escalated] == alt["escalated_ids_desc"]
...
def test_cli_diagnose_subcommand(expected: dict, dossier_text: str):
    ...
    assert data["input_stats"]["object_count"] == expected["object_count"]
    assert data["input_stats"]["unique_object_ids"] == expected["unique_ids"]
    assert data["input_stats"]["buckets"] == expected["buckets"]
...
def test_repair_repatches_reset_workflow_with_custom_output_dir(
    ...
    try:
        ...
        assert diagnosis["output_paths"]["bucket_matrix_json"] == str(custom_dir / "bucket_matrix.json")
...
def test_dismissed_string_normalization_excludes_signal():
    ...
    escalated = _compute_escalated(events)
    assert [row["object_id"] for row in escalated] == ["m3"]


def test_escalated_sort_tie_breaks_by_severity_then_object_id():
    events = [
        {
            "object_id": "c2",
            "accessed_ms": 500,
    ...
    escalated = _compute_escalated(events)
    assert [row["object_id"] for row in escalated] == ["c1", "c2", "h1"]


def test_pipeline_coerces_accessed_ms_and_normalizes_outputs(tmp_path_factory):
    events = [
        {
            "object_id": "p1",
            "accessed_ms": " 200 ",
    ...
    matrix = json.loads((out_dir / "bucket_matrix.json").read_text())

    assert summary["buckets"] == ["core"]
    assert summary["escalated_count"] == 2
    assert summary["dismissed_excluded_count"] == 1
    assert [row["object_id"] for row in escalated] == ["p1", "p2"]
    assert [row["accessed_ms"] for row in escalated] == [200, 0]
...
def test_pipeline_dedupe_tie_break_prefers_non_dismissed_then_detector(tmp_path_factory):
    ...
    assert [row["object_id"] for row in escalated] == ["d1"]
...
def test_override_compaction_and_scope_exercised(tmp_path_factory):
    ...
    try:
        override_rows = [
            {"bucket": "edge", "severity_scope": "high", "start_ms": 100, "end_ms": 160},
            {"bucket": "edge", "severity_scope": "high", "start_ms": 160, "end_ms": 200},
            {"bucket": "edge", "severity_scope": "all", "start_ms": 220, "end_ms": 260},
            {"bucket": "edge", "severity_scope": "debug", "start_ms": 0, "end_ms": 1},
        ]
        ...
        assert [row["object_id"] for row in escalated] == ["o4", "o2"]
    finally:
        OVERRIDES_PATH.write_text(original_overrides)


def test_chain_correlation_is_transitive_across_console_objects(tmp_path_factory):
