"""Diagnostic and repair CLI for Bucketwatch signal workflow."""
...
ISSUE_META = {
    "wrong_source_field": {
        "severity": "critical",
        "description": "Signal rows use accessed_at instead of accessed_ms.",
        "resolution": "Use accessed_ms when emitting signal rows.",
    },
    ...
    "recency_order": {
        ...
        "resolution": "Sort signals by accessed_ms descending (reverse=True).",
    },
    ...
    "dedupe_event": {
        "severity": "high",
        "description": "Duplicate object_id rows are exported multiple times.",
        "resolution": "dedupe object_id rows keeping the highest accessed_ms before export.",
    },
    "benign_filter": {
        "severity": "high",
...
def input_stats(events: list[dict]) -> dict:
    buckets = sorted({str(event.get("bucket", "")).strip().lower() for event in events})
    return {
...
def build_repair_action(issue_id: str, terms: list[str]) -> str:
    templates = {
        "wrong_source_field": "Use accessed_ms when emitting signal rows.",
        "risk_threshold_filter": "Include high and critical rows in signal export.",
        "recency_order": "Sort with reverse=True on accessed_ms for recency-first ordering.",
        "risk_class_normalization": "Normalize severity values using .lower() in canonicalization.",
        "dedupe_event": "dedupe object_id rows keeping the highest accessed_ms before export.",
...
def build_diagnosis_report(
    ...
    if summary is not None and output_dir is not None:
        ...
        report["output_paths"] = {
            ...
            "bucket_matrix_json": str(output_dir / "bucket_matrix.json"),
        }
    return report
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Bucketwatch signal diagnostic CLI")
